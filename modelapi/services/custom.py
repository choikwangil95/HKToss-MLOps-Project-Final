...
def get_news_data_from_db(news_id: str, db: Session) -> dict:
    ...
    query_result = (
        ...
        .join(NewsModel_v2_External, NewsModel_v2.news_id == NewsModel_v2_External.news_id)
        .filter(NewsModel_v2.news_id == news_id)
        .first()
    )
    
    if not query_result:
        raise ValueError(f"뉴스 ID {news_id}를 찾을 수 없거나 필요한 데이터가 누락되었습니다.")
    ...
    result = {
        ...
        'd_minus_5_date_institution': float(query_result.d_minus_5_date_institution or 0),
        'd_minus_4_date_institution': float(query_result.d_minus_4_date_institution or 0),
        'd_minus_3_date_institution': float(query_result.d_minus_3_date_institution or 0),
        'd_minus_2_date_institution': float(query_result.d_minus_2_date_institution or 0),
        'd_minus_1_date_institution': float(query_result.d_minus_1_date_institution or 0),
